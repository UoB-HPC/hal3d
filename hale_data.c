...
size_t init_hale_data(HaleData* hale_data, UnstructuredMesh* umesh) {
  ...
  allocated +=
      allocate_data(&hale_data->subcell_momentum_x, hale_data->nsubcells * 2);
  allocated +=
      allocate_data(&hale_data->subcell_momentum_y, hale_data->nsubcells * 2);
  allocated +=
  ...
  init_subcells_to_subcells(
      ...
      hale_data->subcell_to_neighbour_face);
  ...
  for (int ii = 0; ii < nz; ++ii) {
    for (int jj = 0; jj < ny; ++jj) {
      for (int kk = 0; kk < nx; ++kk) {
        ...
        // Right subcells
        hale_data->subcells_to_nodes[(c_off + 48)] = NODE_IND(ii, jj, kk + 1);
        ...
        // Top subcells
        hale_data->subcells_to_nodes[(c_off + 64)] = NODE_IND(ii, jj + 1, kk);
        ...
        // Back subcells
        hale_data->subcells_to_nodes[(c_off + 80)] = NODE_IND(ii + 1, jj, kk);
