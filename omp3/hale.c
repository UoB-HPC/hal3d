...
#pragma omp parallel for reduction(+ : total_mass)
  for (int cc = 0; cc < ncells; ++cc) {
    ...
    for (int nn = 0; nn < nnodes_by_cell; ++nn) {
      ...
      const double node_c_x = nodes_x[(node_c_index)];
      const double node_c_y = nodes_y[(node_c_index)];

      // Get the midpoints between l and r nodes and current node
      const double node_l_x = 0.5 * (nodes_x[node_l_index] + node_c_x);
      const double node_l_y = 0.5 * (nodes_y[node_l_index] + node_c_y);
      const double node_r_x = 0.5 * (node_c_x + nodes_x[node_r_index]);
      const double node_r_y = 0.5 * (node_c_y + nodes_y[node_r_index]);
...
void initialise_cell_centroids(const int ncells, const int* cells_offsets,
                               ...
                               const double* nodes_y, double* cell_centroids_x,
...
// Initialises the centroids for each cell
void initialise_sub_cell_centroids(const int ncells, const int* cells_offsets,
