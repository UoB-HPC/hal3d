  ...
  for (int cc = 0; cc < ncells; ++cc) {
    ...
    for (int ff = 0; ff < nfaces_by_cell; ++ff) {
      ...
      for (int nn = 0; nn < nnodes_by_face; ++nn) {
        ...
        A_XYZ += 0.5 * (nodes_x0[(node_index)] + nodes_x0[(node_r_index)]) *
                 (nodes_y0[(node_r_index)] - nodes_y0[(node_index)]);
        A_YZX += 0.5 * (nodes_y0[(node_index)] + nodes_y0[(node_r_index)]) *
                 (nodes_z0[(node_r_index)] - nodes_z0[(node_index)]);
        A_ZXY += 0.5 * (nodes_z0[(node_index)] + nodes_z0[(node_r_index)]) *
