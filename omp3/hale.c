    ...
    for (int nn = 0; nn < nnodes; ++nn) {
      ...
      for (int cc = 0; cc < ncells_by_node; ++cc) {
        ...
        const int nnodes_by_cell =
            cells_offsets[(cell_index + 1)] - cell_offset;
...
#pragma omp parallel for simd
    ...
    calculate_artificial_viscosity(
        ...
        cell_centroids_y, velocity_x0, velocity_y0, nodal_soundspeed,
        nodal_mass, nodal_volumes, limiter, node_force_x, node_force_y,
        node_force_x2, node_force_y2);
...
#pragma omp parallel for
    ...
    calculate_artificial_viscosity(
        ...
        cell_centroids_y, velocity_x1, velocity_y1, nodal_soundspeed,
        nodal_mass, nodal_volumes, limiter, node_force_x, node_force_y,
        node_force_x2, node_force_y2);
    ...
    for (int nn = 0; nn < nnodes; ++nn) {
      ...
      for (int cc = 0; cc < ncells_by_node; ++cc) {
        ...
        const double grad_velocity_mag =
            sqrt(grad_velocity_x * grad_velocity_x +
                 grad_velocity_y * grad_velocity_y);
        const double grad_velocity_unit_x =
            (grad_velocity_x != 0.0) ? grad_velocity_x / grad_velocity_mag
                                     : 0.0;
        const double grad_velocity_unit_y =
            (grad_velocity_y != 0.0) ? grad_velocity_y / grad_velocity_mag
                                     : 0.0;
...
#pragma omp parallel for
  ...
  void initialise_mesh_mass(
      const int ncells, const int* cells_offsets,
      const double* cell_centroids_x, const double* cell_centroids_y,
      const int* cells_to_nodes, const double* density0, const double* nodes_x0,
      const double* nodes_y0, double* cell_mass, double* sub_cell_volume,
...
#pragma omp parallel for reduction(+ : total_mass)
  ...
  void initialise_cell_centroids(
      const int ncells, const int* cells_offsets, const int* cells_to_nodes,
      const double* nodes_x0, const double* nodes_y0, double* cell_centroids_x,
