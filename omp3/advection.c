...
void advection_phase(UnstructuredMesh* umesh, HaleData* hale_data) {
  ...
  // Advects mass and energy through the subcell faces using swept edge approx
  perform_momentum_advection(
      umesh->ncells, umesh->cells_offsets, umesh->nodes_x0, umesh->nodes_y0,
      umesh->nodes_z0, hale_data->rezoned_nodes_x, hale_data->rezoned_nodes_y,
      hale_data->rezoned_nodes_z, umesh->cells_to_nodes,
      umesh->faces_to_nodes_offsets, umesh->faces_to_nodes,
      umesh->faces_cclockwise_cell, umesh->faces_to_cells0,
      umesh->faces_to_cells1, hale_data->subcells_to_faces_offsets,
      ...
      hale_data->subcell_volume, hale_data->subcell_momentum_flux_x,
...
#pragma omp parallel for
  for (int cc = 0; cc < ncells; ++cc) {
    ...
    // Consider all faces attached to cell
    for (int ff = 0; ff < nfaces_by_cell; ++ff) {
      ...
      // We don't flux out of a boundary
      if (neighbour_cc == -1) {
        continue;
      }

      int nfaces_by_se = 2 + nnodes_by_face;
      int se_to_faces[nfaces_by_se];
      for (int ii = 0; ii < nfaces_by_se; ++ii) {
        se_to_faces[(ii)] = ii;
      }

      double se_nodes_x[2 * nnodes_by_face];
      double se_nodes_y[2 * nnodes_by_face];
      double se_nodes_z[2 * nnodes_by_face];
      for (int ii = 0; ii < nnodes_by_face; ++ii) {
        const int n = faces_to_nodes[(face_to_nodes_off + ii)];
        se_nodes_x[(ii)] = nodes_x[(n)];
        se_nodes_y[(ii)] = nodes_y[(n)];
        se_nodes_z[(ii)] = nodes_z[(n)];
        se_nodes_x[(ii + nnodes_by_face)] = rezoned_nodes_x[(n)];
        se_nodes_y[(ii + nnodes_by_face)] = rezoned_nodes_y[(n)];
        se_nodes_z[(ii + nnodes_by_face)] = rezoned_nodes_z[(n)];
      }

      int se_faces_to_nodes[6 * nnodes_by_face];
      int off = 0;
      for (int ii = 0; ii < 2 * nnodes_by_face; ++ii) {
        se_faces_to_nodes[(off++)] = ii;
      }
      for (int ii = 0; ii < nnodes_by_face; ++ii) {
        se_faces_to_nodes[(off++)] = ii;
        se_faces_to_nodes[(off++)] = ii + nnodes_by_face;
        se_faces_to_nodes[(off++)] = ii + 1 + nnodes_by_face;
        se_faces_to_nodes[(off++)] = ii + 1;
      }

      off = 0;
      int se_faces_to_nodes_offsets[nfaces_by_se + 1];
      se_faces_to_nodes_offsets[(off++)] = 0;
      se_faces_to_nodes_offsets[(off++)] = nnodes_by_face;
      se_faces_to_nodes_offsets[(off++)] = 2 * nnodes_by_face;
      for (int ii = 0; ii < nnodes_by_face; ++ii) {
        se_faces_to_nodes_offsets[(off + ii)] =
            se_faces_to_nodes_offsets[(off + ii - 1)] + 4;
      }
      ...
      calc_centroid(nnodes_by_face, rezoned_nodes_x, rezoned_nodes_y,
                    ...
                    &rz_face_c);
      calc_centroid(2 * nnodes_by_face, se_nodes_x, se_nodes_y, se_nodes_z,
                    se_faces_to_nodes, 0, &swept_edge_c);

      // Calculate the volume of the swept edge prism
      double swept_edge_vol = 0.0;
      calc_volume(0, 2 + nnodes_by_face, se_to_faces, se_faces_to_nodes,
                  se_faces_to_nodes_offsets, se_nodes_x, se_nodes_y, se_nodes_z,
      ...
      // Check if mass flowing into or out of the the current cell
      vec_t ab = {rz_face_c.x - face_c.x, rz_face_c.y - face_c.y,
                  rz_face_c.z - face_c.z};
      vec_t ac = {cell_c.x - face_c.x, cell_c.y - face_c.y,
                  cell_c.z - face_c.z};
      const int is_outflux = (ab.x * ac.x + ab.y * ac.y + ab.z * ac.z > 0.0);
      const int sweep_cell_index = (is_outflux ? cc : neighbour_cc);

      const int sweep_cell_to_nodes_off = cells_offsets[(sweep_cell_index)];
      const int nnodes_by_sweep_cell =
          cells_offsets[(sweep_cell_index + 1)] - sweep_cell_to_nodes_off;
      vec_t sweep_cell_c = {0.0, 0.0, 0.0};
      calc_centroid(nnodes_by_sweep_cell, nodes_x, nodes_y, nodes_z,
                    cells_to_nodes, sweep_cell_to_nodes_off, &sweep_cell_c);

      /* CALCULATE THE SWEEP CELL GRADIENTS FOR MASS AND ENERGY */
      ...
      const double sweep_cell_density = density[(sweep_cell_index)];
      ...
      const int nfaces_by_neighbour =
          cells_to_faces_offsets[(sweep_cell_index + 1)] -
      ...
      for (int ff2 = 0; ff2 < nfaces_by_neighbour; ++ff2) {
        ...
        const int sweep_neighbour_index =
            (faces_to_cells0[(neighbour_face_index)] == sweep_cell_index)
        ...
        double neighbour_vol = cell_volume[(sweep_neighbour_index)];
        vec_t i = {cell_centroids_x[(sweep_neighbour_index)] * neighbour_vol -
                       sweep_cell_c.x * neighbour_vol,
                   cell_centroids_y[(sweep_neighbour_index)] * neighbour_vol -
                       sweep_cell_c.y * neighbour_vol,
                   cell_centroids_z[(sweep_neighbour_index)] * neighbour_vol -
                       sweep_cell_c.z * neighbour_vol};
        ...
        const double neighbour_m_density = density[(sweep_neighbour_index)];
        const double dneighbour_m_density =
            neighbour_m_density - sweep_cell_density;
        ...
        const double neighbour_ie_density =
            energy[(sweep_neighbour_index)] * density[(sweep_neighbour_index)];
      ...
      // Performing the limiting actually requires the sweep cell's nodes
      double m_limiter = 1.0;
      double ie_limiter = 1.0;
      double ke_limiter = 1.0;

      // The sweep cell always includes the nodes of the sweep face
      for (int nn = 0; nn < nnodes_by_sweep_cell; ++nn) {
        const int node_index = cells_to_nodes[(sweep_cell_to_nodes_off + nn)];
        m_limiter =
            min(m_limiter,
                calc_cell_limiter(sweep_cell_density, gmax_m, gmin_m, &grad_m,
                                  nodes_x[(node_index)], nodes_y[(node_index)],
                                  nodes_z[(node_index)], &sweep_cell_c));
        ie_limiter = min(
            ie_limiter,
            calc_cell_limiter(sweep_cell_ie_density, gmax_ie, gmin_ie, &grad_ie,
                              nodes_x[(node_index)], nodes_y[(node_index)],
                              nodes_z[(node_index)], &sweep_cell_c));
        ke_limiter = min(
            ke_limiter,
            calc_cell_limiter(sweep_cell_ke_density, gmax_ke, gmin_ke, &grad_ke,
                              nodes_x[(node_index)], nodes_y[(node_index)],
                              nodes_z[(node_index)], &sweep_cell_c));
      }
      ...
      const double local_mass_flux =
          swept_edge_vol *
          (sweep_cell_density + grad_m.x * (swept_edge_c.x - sweep_cell_c.x) +
           grad_m.y * (swept_edge_c.y - sweep_cell_c.y) +
           grad_m.z * (swept_edge_c.z - sweep_cell_c.z));

      // Calculate the flux for internal energy density in the cell
      const double local_ie_flux =
          swept_edge_vol * (sweep_cell_ie_density +
                            grad_ie.x * (swept_edge_c.x - sweep_cell_c.x) +
                            grad_ie.y * (swept_edge_c.y - sweep_cell_c.y) +
                            grad_ie.z * (swept_edge_c.z - sweep_cell_c.z));

      // Calculate the flux for kinetic energy density in the cell
      const double local_ke_flux =
          swept_edge_vol * (sweep_cell_ke_density +
                            grad_ke.x * (swept_edge_c.x - sweep_cell_c.x) +
                            grad_ke.y * (swept_edge_c.y - sweep_cell_c.y) +
                            grad_ke.z * (swept_edge_c.z - sweep_cell_c.z));

      // Mass and energy are either flowing into or out of the cell
      if (is_outflux) {
        mass_flux[(cc)] += local_mass_flux;
        ie_mass_flux[(cc)] += local_ie_flux;
        ke_mass_flux[(cc)] += local_ke_flux;
      } else {
        mass_flux[(cc)] -= local_mass_flux;
        ie_mass_flux[(cc)] -= local_ie_flux;
        ke_mass_flux[(cc)] -= local_ke_flux;
  ...
  for (int cc = 0; cc < ncells; ++cc) {
    ...
    for (int nn = 0; nn < nnodes_by_cell; ++nn) {
      ...
      for (int ff = 0; ff < nfaces_by_subcell; ++ff) {
        ...
        // for testing purposes in order to ensure that no flux is
        // inadvertently
