...
void scatter_phase(const int ncells, const int nnodes, vec_t* initial_momentum,
                   ...
                   const double* nodes_z, double* cell_volume, double* energy,
                   double* density, double* velocity_x, double* velocity_y,
                   double* velocity_z, double* cell_mass, double* nodal_mass,
  ...
  scatter_energy_and_mass(
      ncells, nodes_x, nodes_y, nodes_z, cell_volume, energy, density,
      cell_mass, subcell_ie_mass, subcell_mass, subcell_ie_mass_flux,
      subcell_mass_flux, faces_to_nodes, faces_to_nodes_offsets,
      cells_to_faces_offsets, cells_to_faces, cells_to_nodes_offsets,
      cells_to_nodes, initial_mass, initial_ie_mass);
...
#pragma omp parallel for reduction(                                            \
  ...
  for (int nn = 0; nn < nnodes; ++nn) {
    ...
    for (int cc = 0; cc < ncells_by_node; ++cc) {
      ...
      total_momentum_x += new_subcell_momentum_x;
      total_momentum_y += new_subcell_momentum_y;
      total_momentum_z += new_subcell_momentum_z;
