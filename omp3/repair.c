  ...
  for (int cc = 0; cc < ncells; ++cc) {
    ...
    for (int nn = 0; nn < nnodes_by_cell; ++nn) {
      ...
      if (dm_need_receive > 0.0) {
        subcell_mass[(subcell_index)] = gmin_m * vol;
        // Loop over neighbours
        for (int ss = 0; ss < nsubcell_neighbours; ++ss) {
          const int neighbour_index =
              subcells_to_subcells[(subcell_to_subcells_off + ss)];
          subcell_mass[(neighbour_index)] -=
              (dm_avail_donate_local[(ss)] / dm_avail_donate) * dm_need_receive;
        }

      } else if (dm_need_donate > 0.0) {
        subcell_mass[(subcell_index)] = gmax_m * vol;
        ...
        for (int ss = 0; ss < nsubcell_neighbours; ++ss) {
          const int neighbour_index =
              subcells_to_subcells[(subcell_to_subcells_off + ss)];
          subcell_mass[(neighbour_index)] +=
              (dm_avail_receive_local[(ss)] / dm_avail_receive) *
              dm_need_donate;
        }
      }
